...
class WheelchairEnv(gym.Env):
    def __init__(self, env_id: int, noise_level: float = 0.05):
        ...
        # CORREÇÃO: IP alterado para 127.0.0.1 para evitar erros de socket no Windows
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, dict]:
        ...
        action_converted = self.to_action(action)
        
        obs = self.send_action_get_obs(action_converted)
        reward = self.get_reward(obs.lidar, action_converted)
        ...
        # Adequado ao standard do Gymnasium: obs, reward, terminated, truncated, info
    ...
    def get_observation(self) -> RobotState:
        ...
        # Interpolação automática: Aceita qualquer número de raios e transforma em 360 para a CNN
        ...
        # Injeção controlada de ruído gaussiano (Fase 1 e Fase 2)
